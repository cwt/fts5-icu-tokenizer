/**
 * @file fts5_icu_legacy.c
 * @brief A dynamic FTS5 tokenizer for SQLite that uses ICU to segment text
 *		based on a locale provided at compile time (Legacy API v1).
 *
 * This version implements the older FTS5 API v1 specification for compatibility
...
// Forward declarations for legacy v1 API
static int icuCreate(void*, const char**, int, Fts5Tokenizer**);
static void icuDelete(Fts5Tokenizer*);
static int icuTokenize(Fts5Tokenizer*, void*, int, const char*, int,
                       int (*)(void*, int, const char*, int, int, int));

// Main tokenizer struct for legacy v1 API
 ...
 * @return SQLITE_OK on success, appropriate error code on failure
 */
static int process_single_token(IcuTokenizerV1* pTokenizer, UChar* pUText, int32_t textBufferSize,
...
int PASTE(sqlite3_ftsicu, INIT_LOCALE_SUFFIX_FOR_FUNCTION, legacy,
    ...
    if (rc != SQLITE_OK) {
        *pzErrMsg = sqlite3_mprintf("Failed to register ICU tokenizer (legacy): %s",
                                    sqlite3_errstr(rc));
    }
    return rc;
}